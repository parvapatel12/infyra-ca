...
import { X, Download, TrendingUp, Users, ShoppingBag, Instagram, Mail, RefreshCw, BarChart2, MousePointerClick } from 'lucide-react';
...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ onClose }) => {
  const [summary, setSummary] = useState<AnalyticsSummary>(AnalyticsService.getSummary());
  const [events, setEvents] = useState<AnalyticsEvent[]>(AnalyticsService.getEvents());
  const [leads, setLeads] = useState<LeadSubscriber[]>(AnalyticsService.getLeads());
  ...
  const refreshData = () => {
    ...
    setLeads(AnalyticsService.getLeads());
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/80 backdrop-blur-md overflow-y-auto">
      <div className="bg-slate-900 text-white w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-700 overflow-hidden relative my-8 my-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-950/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => AnalyticsService.exportAnalyticsCSV()}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700 space-y-4">
                ...
                <div className="space-y-3">
                  {Object.keys(summary.productClicks).length === 0 ? (
                    ...
                    {events.length === 0 ? (
                      ...
                      events.map(e => (
                        <tr key={e.id} className="hover:bg-slate-800/50">
                          <td className="p-3 font-mono text-slate-400">{new Date(e.timestamp).toLocaleTimeString()}</td>
                          <td className="p-3">
                          ...
                          <td className="p-3 uppercase text-[10px]">{e.deviceType}</td>
                          <td className="p-3 text-slate-400 truncate max-w-xs">{e.referrer}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
                    ...
                    {leads.length === 0 ? (
                      ...
                      leads.map(l => (
                        <tr key={l.id} className="hover:bg-slate-800/50">
                          ...
                          <td className="p-3 font-mono text-slate-400">{new Date(l.subscribedAt).toLocaleString()}</td>
